...
export const ClusterTable = () => {
  ...
  const onDelete = async (id: string) => {
    await dialogs.confirm(<div className='flex flex-col'>
      ...
      <TextField value={clusterId} onChange={(e) => setClusterId(e.target.value)} sx={{ mt: 2 }} id="standard-basic" label={t('gateway.table.clusterId')} variant="standard" />
    </div>, {
      title: <span className='text-xl font-bold'>{t('common.notification')}</span>,
      okText: <Button loading={deleteCluster.isPending} onClick={async () => {
        if (clusterId === id) {
    ...
    () => [
      ...
      columnHelper.accessor('sessionAffinity', {
        ...
        cell: (info) => {
          ...
          return <Button>{sessionAffinity.enabled ? t('common.enabled') : t('common.disabled')}</Button>
        },
      ...
      columnHelper.accessor('healthCheck', {
        ...
        cell: (info) => {
          ...
          return <Button>{healthCheck.active?.enabled ? t('common.enabled') : t('common.disabled')}</Button>
        },
        size: columnSizes.status,
      }),
      columnHelper.accessor('httpClient', {
        ...
        cell: (info) => {
          ...
          return <Button>{httpClient.dangerousAcceptAnyServerCertificate ? t('common.enabled') : t('common.disabled')}</Button>;
        },
        size: columnSizes.status,
      }),
      columnHelper.accessor('httpRequest', {
        ...
        cell: (info) => {
          ...
          return <Button>{request ? request.version : t('common.disabled')}</Button>
        },
        size: columnSizes.status,
      }),
      columnHelper.accessor('destinations', {
        ...
        cell: (info) => {
          ...
          return destinations ? Object.entries(destinations).map(([key, dest]) => (
      ...
      columnHelper.accessor('metadata', {
        ...
        cell: (info) => {
          ...
          return metadata ? Object.keys(metadata).length > 0 ? t('common.enabled') : t('common.disabled') : t('common.disabled');
        },
        size: columnSizes.status,
      }),
        ...
        cell: (info) => (
          <Box display="flex" justifyContent="center" gap={1}>
            <Tooltip title={t('gateway.actions.edit')}>
              <IconButton
                size="small"
                onClick={() => onEdit(info.row.original.clusterId)}
              >
                <Edit2 size={18} />
              </IconButton>
            </Tooltip>
